 ...
 *
 * The reference is explicit: JARVIS from Iron Man. Calm, always-aware,
...
/**
 * How JARVIS addresses the operator. `JARVIS_ADDRESS_STYLE` selects the mode;
 * the names come from `JARVIS.address`.
 */
...
export function pickOpener(
  kind: keyof typeof JARVIS.openers,
  seed: number,
): string {
  const bank = JARVIS.openers[kind]
 ...
 */
export function enforceJarvisVoice(input: string): string {
 ...
 * Clamped to `maxSpokenChars` (a knob on `jarvis.voice.transcribe`).
 */
export function spokenSummary(narration: string, maxChars = 320): string {
  const firstSentence = narration
    ...
    .replace(/\|[^\n]*\|/g, '')
    .split(/(?<=[.?])\s/)[0]
  const clean = enforceJarvisVoice(firstSentence ?? narration)
