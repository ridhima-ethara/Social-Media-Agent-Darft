...
const SCRAPING_SKILLS: SkillSpec[] = [
  ...
  },
  {
    ...
    summary:
      'Checks whether the crawl4ai sidecar is reachable and reports what will answer this run, naming the reason when it will not.',
    inputs: ['crawl4ai configuration', 'The source registry'],
    ...
    config: [
      ...
      bool('failIfNoSource', 'Fail when nothing is reachable', false,
        'On, a run stops at the source check when crawl4ai is not configured rather than proceeding to four empty lanes. Off lets the run continue and report the emptiness lane by lane.'),
    ],
  },
  {
    ...
    summary:
      'The crawl4ai call. Searches every enabled platform lane and the open web for each keyword, scores what comes back against the brand topics and the Knowledge Base, and admits only what aligns. A lane that returns nothing is reported, never substituted.',
    inputs: ['Resolved keywords', 'Brand topics', 'The Knowledge Base'],
    outputs: ['Captured pages, stamped with their platform and alignment'],
    order: 3,
    enabledByDefault: true,
    critical: true,
    config: [
      num('maxItemsPerKeyword', 'Pages per keyword, per lane', 8,
        'The ceiling on pages crawled for each keyword on each enabled lane. Every page is a real browser load on this machine, so this is the single biggest driver of run time.',
        { min: 1, max: 40, step: 1 }),
