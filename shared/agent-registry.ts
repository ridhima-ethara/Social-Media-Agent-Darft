...
export const AGENTS: AgentSpec[] = [
  ...
  },
  {
    ...
    name: 'Scraping Agent',
    stage: 'discover',
    role: 'Signal capture from LinkedIn',
    description:
...
const SCRAPING_SKILLS: SkillSpec[] = [
  ...
  },
  {
    ...
    config: [
      ...
      bool('failIfNoSource', 'Fail when nothing is reachable', false,
        'On, a run with no live source stops instead of falling back to the bundled corpus. Off is what makes the product demoable with an empty environment.'),
    ],
  },
  {
    ...
    inputs: ['Resolved keywords', 'Run mode'],
    outputs: ['Raw posts'],
    order: 3,
    enabledByDefault: true,
    critical: true,
    config: [
      num('maxItemsPerKeyword', 'Items per keyword', 50,
        'The ceiling on posts pulled for each keyword. The single biggest driver of both signal quality and scrape cost.',
        { min: 5, max: 200, step: 5 }),
      enumField('datePosted', 'Recency window', ['past-24h', 'past-week', 'past-month'], 'past-week',
        'How far back to look. Past week is the balance point: enough volume to rank, recent enough to be a trend.'),
      enumField('sortBy', 'Sort order', ['relevance', 'date'], 'date',
        'Date surfaces what is moving now. Relevance surfaces the strongest match regardless of age.'),
      num('retries', 'Retries per keyword', 2,
        'How many times a failed scrape is retried with exponential backoff before the keyword falls back to fixtures.',
        { min: 0, max: 5, step: 1 }),
      num('minAuthorFollowers', 'Minimum author followers', 0,
        'Drops posts from accounts below this size. Raise it to bias toward established voices; zero keeps everything.',
  ...
  },
  {
    ...
    enabledByDefault: true,
    config: [
      num('expandTop', 'Hashtags to expand', 15,
        'How many of the top candidates get their own feed scraped. Each is an extra call, so this trades accuracy against run time.',
        { min: 0, max: 50, step: 1 }),
      num('itemsPerHashtag', 'Items per hashtag feed', 25,
        'How deep to read each hashtag\u2019s own feed.',
        { min: 5, max: 100, step: 5 }),
  ...
  },
  {
    ...
    outputs: ['Competitor posts'],
    order: 7,
    enabledByDefault: true,
    config: [
      enumField('tier', 'Which competitors', ['P0 only', 'P0+P1', 'All'], 'P0+P1',
        'P0 is the direct set. P0+P1 adds the adjacent labs. All includes the long tail, which is mostly noise.'),
      num('postsPerCompetitor', 'Posts per competitor', 3,
        'How many recent posts to read from each page.',
        { min: 1, max: 20, step: 1 }),
    ],
  },
  {
  ...
  },
  {
    ...
    config: [
      enumField('closeStyle', 'Close style',
        ['Implication', 'Open question', 'Forward look', 'None'], 'Implication',
        'Implication states what follows from the finding. Open question invites replies. None ends on the evidence.'),
  ...
  },
  {
    ...
    config: [
      ...
      num('retries', 'Retries per hashtag', 2,
        'How many times a failed research call is retried before that hashtag falls back to fixtures.',
        { min: 0, max: 5, step: 1 }),
    ],
  },
  {
