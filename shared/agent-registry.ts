...
export const AGENTS: AgentSpec[] = [
  ...
  },
  {
    ...
    icon: 'search',
    stage: 'discover',
    role: 'Scraping Agent · Platform-level trend discovery on LinkedIn, Instagram, Facebook and X',
    description:
  ...
  },
  {
    ...
    consumes: ['Validated items', 'Validated hashtags', 'Competitor posts', 'SocialFetch data on Ethara.AI\u2019s own channels'],
    produces: ['Ranked opportunities', 'The top 25 hashtag set', 'Format and angle recommendations', 'The Social Media Listener report'],
    handsOffTo: ['calendar'],
    sections: ['Clustering', 'Judgement', 'Consolidation', 'Social listening'],
  },
  {
    id: 'calendar',
    name: 'Dora',
    icon: 'calendar',
    stage: 'plan',
    role: 'Calendar Agent · Topics on the calendar, posts only when due',
    description:
...
const SCRAPING_SKILLS: SkillSpec[] = [
  ...
  },
  {
    ...
    inputs: ['Claude Bridge configuration', 'The source registry'],
    ...
    config: [
      ...
      bool('failIfNoSource', 'Fail when nothing is reachable', false,
        'On, a run stops at the source check when the Claude Bridge cannot run rather than proceeding to five empty lanes. Off lets the run continue and report the emptiness lane by lane.'),
    ],
  },
  {
    ...
    inputs: ['Resolved keywords', 'Ethara brand context', 'The Knowledge Base'],
    outputs: ['Platform trends (topic, hashtags, post URLs, dates, authors, matched keywords, reason)', 'Captured posts, newest first, stamped with their platform and alignment'],
    order: 3,
    enabledByDefault: true,
    critical: true,
    config: [
      num('maxSearchesPerPlatform', 'Searches per platform', 16,
        'The searches the Claude Bridge runs on each platform per capture — ONE TOPIC PER SEARCH, with the current month named ("RLVR September 2026"), because an OR of quoted phrases returns almost nothing recent. Half go to the highest-priority keywords every run, the rest rotate daily through every other keyword, plus the research-corpus topics, one broad field term and one hashtag search. Each search costs roughly three cents of Claude usage; sixteen on four platforms is about two dollars a run. Thirty is the hard ceiling in the bridge configuration.',
        { min: 1, max: 30, step: 1 }),
      num('maxPostsPerTrend', 'Posts per trend', 5,
        'The most posts kept for any one trend, newest first. Five is also the hard ceiling in the bridge configuration.',
        { min: 1, max: 5, step: 1 }),
      ...
      enumField('datePosted', 'Recency window',
        ['current-month', 'past-24h', 'past-48h', 'past-week', 'past-month', 'past-quarter'], 'current-month',
        'How far back discovery looks. The default, the current month, is what is trending THIS MONTH: posts from midnight on the 1st (workspace time zone) to now, with posts from today labelled and shown first. The others are rolling windows. A post is kept only when its date can be verified inside the window (decoded from its post id), so an undated post is left out. Search engines index social posts late — often by days — so early in a month the window is thin; that is reported, and with \u201cShow the newest posts when the window is empty\u201d on, the newest older posts are listed, labelled, never presented as current.'),
      bool('learnNewHashtags', 'Learn new hashtags into the Knowledge Base', true,
        'On, hashtags found on validated, Ethara-relevant posts that Ethara does not track yet are saved to the Knowledge Base as \u201cDiscovered Hashtag\u201d entries (citing the posts), and the next discovery run searches the strongest of them too \u2014 so the search follows what is trending instead of only the configured keywords. Off, nothing is learned.'),
      num('maxNewHashtagsPerRun', 'New hashtags learned per run', 10,
      ...
      bool('includeLinkedin', 'Capture LinkedIn', true,
        'Discovers trends in LinkedIn posts through the Claude Bridge; each post is dated from its id. The primary publishing surface.'),
      bool('includeInstagram', 'Capture Instagram', true,
        'Discovers trends in Instagram posts and reels through the Claude Bridge; each is dated from its shortcode. Search engines index little of Instagram, so expect thin results, reported as such.'),
      bool('includeX', 'Capture X', true,
        'Discovers trends in X posts through the Claude Bridge; each is dated from its status id.'),
      bool('includeFacebook', 'Capture Facebook', true,
        'Facebook through the Claude Bridge. Facebook post ids carry no date, so no post can be verified inside the window; the platform is skipped, with that reason, rather than spending searches it cannot use.'),
  ...
  },
  {
    ...
    config: [
      ...
      num('itemsPerHashtag', 'Pages per hashtag', 4,
        ...
        { min: 1, max: 25, step: 1 }),
      bool('enabled', 'Expand at all', false,
        'Off by default: platform trend discovery already searches the leading keywords as hashtags within its per-platform search budget, and each expansion would be an extra search. On, the strongest tags are read again as search terms of their own.'),
    ],
  },
  {
...
const ANALYSIS_SKILLS: SkillSpec[] = [
  ...
  },
  {
    ...
    config: [
      ...
      bool('dropNonEditorial', 'Reject other people\u2019s announcements', true,
        'The corpus is other accounts\u2019 posts, so it contains job ads, certification announcements, personal product reviews and event promotion. Those mention our vocabulary and therefore pass a keyword brand-fit score, which is how \u201cHire a generative AI engineer\u201d reached the calendar. On, a subject that is an announcement about a person or company rather than a finding about the world is rejected before it is scored, and the reason is named. Off lets them through.'),
  ...
  },
  {
    ...
    inputs: ['SocialFetch data: profiles, posts, engagement, comments', 'FetchLayer data: Glassdoor employer rating and reviews'],
    outputs: ['The Social Media Listener report (social channels + Glassdoor)'],
    order: 9,
    enabledByDefault: true,
    critical: false,
    config: [
      text('company', 'Company', 'Ethara.AI',
        'The company the listener reports on. Used in the report and in Claude\u2019s instructions.'),
      text('linkedinPage', 'LinkedIn company page', 'https://www.linkedin.com/company/ethara-ai',
        'The LinkedIn company page URL (or slug). Empty skips LinkedIn.'),
      text('instagramHandle', 'Instagram handle', 'ethara.ai',
        'The Instagram handle or profile URL. Empty skips Instagram.'),
      text('facebookPage', 'Facebook page', 'https://www.facebook.com/people/Ethara-AI/pfbid0MzfHN74R6ouyZYh2YjXbAGnSita1mYANTWFQHmdRYQ7PrkZG5DgmcJZu4eFXy3vel/',
        'The Facebook page URL (Ethara.AI\u2019s page, as supplied by the operator). Empty skips Facebook.'),
      text('xHandle', 'X handle', 'EtharaAi',
        'The X (Twitter) handle, without @. Empty skips X.'),
      bool('includeLinkedin', 'Listen on LinkedIn', true, 'Include LinkedIn in the listener.'),
      bool('includeInstagram', 'Listen on Instagram', true, 'Include Instagram in the listener.'),
      bool('includeFacebook', 'Listen on Facebook', true, 'Include Facebook in the listener.'),
      bool('includeX', 'Listen on X', true, 'Include X in the listener.'),
      num('postsPerPlatform', 'Posts analysed per platform', 10,
        'How many of the most recent posts are read per platform. One list call per platform (LinkedIn\u2019s costs 3 SocialFetch credits, the others 1), whatever this is set to up to a page.',
        { min: 1, max: 30, step: 1 }),
      num('commentPostsPerPlatform', 'Posts whose comments are read', 4,
        'Comments are fetched only for posts that have comments, the most-commented first, up to this many per platform. Each is one SocialFetch call (3 credits on LinkedIn, 1 elsewhere), so this is the main driver of a run\u2019s cost. Zero reads no comments.',
        { min: 0, max: 10, step: 1 }),
      num('commentsPerPost', 'Comments read per post', 20,
        'The most comments read from one post. Comments beyond this are not analysed.',
        { min: 1, max: 50, step: 1 }),
      num('topPostsCount', 'Top posts shown', 3,
        'How many top-performing posts each platform lists.',
        { min: 1, max: 10, step: 1 }),
      num('lowestPostsCount', 'Lowest posts shown', 2,
        'How many lowest-performing posts each platform lists.',
        { min: 0, max: 10, step: 1 }),
      bool('includeReposts', 'Count reposts', true,
        'On, posts the company reposted count as its activity (their engagement is the original post\u2019s). Off analyses only the company\u2019s own posts.'),
      bool('includeGlassdoor', 'Read Glassdoor reviews', true,
        'On, reads Ethara.AI\u2019s Glassdoor employer profile and recent reviews through FetchLayer \u2014 overall rating, recommend %, CEO approval and the recurring pros and cons. Off, or with FETCHLAYER_API_KEY unset, the Glassdoor block reports itself unavailable and the rest of the listener is unaffected.'),
      text('glassdoorEmployer', 'Glassdoor employer', 'Ethara.AI',
        'The employer name, Glassdoor URL or employer id FetchLayer looks up. Empty skips Glassdoor.'),
      num('glassdoorReviewLimit', 'Glassdoor reviews read', 15,
        'How many of the most recent Glassdoor reviews are read \u2014 the sample the pros/cons themes and review sentiment rest on. Each read is a FetchLayer call.',
        { min: 1, max: 50, step: 1 }),
      bool('claudeAnalysis', 'Claude reads comments and writes insights', true,
        'On, Claude classifies each comment (sentiment, question/praise/complaint\u2026, topic) and writes the insights from the computed figures. Off, sentiment is not measured and the insights are computed only.'),
      enumField('claudeModel', 'Claude model', ['sonnet', 'haiku', 'opus'], 'sonnet',
        'The Claude model used for comment readings and insights.'),
      num('claudeBudgetCents', 'Claude budget per call', 50,
        'The most one Claude call may spend, in US cents. A run makes one call per comment batch plus one for the insights.',
        { min: 5, max: 500, step: 5, unit: '¢' }),
      num('claudeBatchSize', 'Comments per Claude call', 40,
        'How many comments Claude reads in one call.',
        { min: 5, max: 100, step: 5 }),
      num('refreshHours', 'Reuse a report younger than', 24,
        'Inside a pipeline run, a listener report younger than this is reused instead of fetching again \u2014 SocialFetch bills per call and the company\u2019s own channels do not change by the minute. The Run listener button always fetches fresh. Zero always fetches.',
        { min: 0, max: 168, step: 1, unit: 'h' }),
    ],
  },
  {
    id: 'analysis.competitor.intel',
    agentId: 'analysis',
    ...
    inputs: ['The Competitor Universe', 'Competitor websites, Wikipedia, dated news, review pages', 'DataForSEO (optional)', 'Ethara product context'],
    outputs: ['Versioned competitor profiles with sources and change logs', 'The cross-competitor market analysis'],
    order: 10,
    enabledByDefault: true,
    critical: false,
    config: [
      enumField('depth', 'Profile depth', ['quick', 'deep'], 'quick',
        'The skill\u2019s two depths. Quick scan: homepage and the first key pages, SEO overview. Deep profile: more pages, top pages and organic competitors from DataForSEO, larger evidence budget.'),
      num('maxPagesPerCompetitor', 'Pages read per competitor', 5,
        'The homepage plus the key pages the skill prioritises (pricing, about, blog, research, changelog, customers, docs, careers), in that order.',
        { min: 1, max: 10, step: 1 }),
      num('newsWindowDays', 'Recent developments window', 30,
        'Only coverage and blog posts dated inside this many days count as recent developments.',
        { min: 7, max: 120, step: 1, unit: 'days' }),
      num('maxNewsItems', 'Dated items per competitor', 8,
        'The most news / blog items passed to the profile as recent-development evidence.',
        { min: 0, max: 20, step: 1 }),
      bool('includeSeo', 'Read SEO data (DataForSEO)', true,
        'On, and with DATAFORSEO_LOGIN / DATAFORSEO_PASSWORD set, reads domain rank, organic keywords and traffic, backlinks and referring domains. Otherwise every SEO figure reads \u201cNot available from current sources\u201d.'),
      bool('includeReviews', 'Read review pages', true,
        'Reads G2 / Capterra / TrustRadius / Product Hunt pages configured in a competitor\u2019s links, where robots.txt allows. None configured, or disallowed, is stated per source.'),
      bool('runMarketAnalysis', 'Cross-competitor analysis', true,
        'After the profiles, analyse the market across every active competitor\u2019s latest profile.'),
      num('concurrency', 'Competitors in parallel', 2,
        'How many competitors are researched at once.',
        { min: 1, max: 4, step: 1 }),
      num('maxCompetitorsPerPipelineRun', 'Due competitors per pipeline run', 0,
        'Inside a pipeline run, profile at most this many competitors that are due by their monitoring frequency. Zero leaves Competitor Intelligence to its own tab and the monitor schedule \u2014 a full universe takes many minutes.',
        { min: 0, max: 25, step: 1 }),
      enumField('claudeModel', 'Claude model', ['sonnet', 'haiku', 'opus'], 'sonnet',
        'The Claude model that writes the profiles and the market analysis.'),
      num('claudeBudgetCents', 'Claude budget per profile', 60,
        'The most one profile\u2019s Claude call may spend, in US cents.',
        { min: 10, max: 500, step: 5, unit: '¢' }),
      num('marketBudgetCents', 'Claude budget for the market analysis', 120,
        'The most the cross-competitor analysis may spend, in US cents.',
        { min: 10, max: 1000, step: 10, unit: '¢' }),
    ],
  },
]
...
const CALENDAR_SKILLS: SkillSpec[] = [
  ...
  },
  {
    ...
    enabledByDefault: true,
    config: [
      pct('similarityThreshold', 'Repeat threshold', 55,
        'How similar a new idea must be to an existing one to count as a repeat.'),
      bool('onePerTopic', 'One idea per trend', true,
        'On, a run keeps only the strongest idea from each trend (source topic), so the same topic never appears twice under two wordings.'),
  ...
  },
  {
    ...
    config: [
      num('planningHorizonDays', 'Planning horizon', 14,
        'How many days ahead the calendar spreads ideas over. 7 plans this week; 14 plans this week and next. The window always ends at a week boundary, so a run mid-week never spills into a third week, and two weeks is the ceiling — the calendar never plans further ahead. Weekends are still skipped when "Avoid weekends" is on.',
        { min: 1, max: 14, step: 1, unit: 'days' }),
  ...
  },
  {
    ...
    config: [
      ...
      num('targetPerWeek', 'Target posts per week per platform', 3,
        ...
        { min: 1, max: 14, step: 1 }),
      num('planningHorizonDays', 'Planning horizon', 14,
        'The window a post displaced from a full day may move within. Declared here as well as on slot optimisation because this skill MOVES dates: it wraps around the window’s postable days instead of walking past its last one. Keep it equal to the slot-optimisation horizon.',
        { min: 1, max: 14, step: 1, unit: 'days' }),
    ],
  },
  {
  ...
  },
  {
    ...
    config: [
      ...
      bool('everyPlatform', 'Plan every platform', true,
        'On, every topic is planned for every platform in play on the same day — LinkedIn, X, Facebook and Instagram — so the week has a recommendation for each channel. The per-platform cap still keeps each platform to its weekly target. Off, only the strongest ideas get a staggered variant (Variants per idea, Stagger).'),
      bool('shareMetaPost', 'Facebook and Instagram share one post', true,
        'On, a topic’s Facebook and Instagram entries carry the same caption: it is written once and the second reuses it (each still gets its own image size).'),
      ...
      num('planningHorizonDays', 'Planning horizon', 14,
        'The window a variant must land inside. A variant staggered past the window’s end is staggered earlier instead, and dropped when neither fits. Keep it equal to the slot-optimisation horizon.',
        { min: 1, max: 14, step: 1, unit: 'days' }),
    ],
  },
  {
    ...
    critical: true,
    config: [
      num('topPerPlatform', 'Calendar slots per platform', 1,
        'How many topics per platform take a date on the week. An idea ranked below this is not placed \u2014 the run reports how many and the cut-off score \u2014 because the calendar keeps no suggestion list.',
        { min: 1, max: 30, step: 1 }),
      num('planningWeeks', 'Weeks the calendar plans for', 1,
      ...
      bool('autoWriteCalendar', 'Write the post-ready posts automatically', true,
        'On, a topic placed on a post-ready date (see \u201cPosts written ahead\u201d) is handed straight to the Content and Image Agents, so today\u2019s post is ready without anyone asking. Topics on later dates are never written automatically, whatever this is set to: they wait in the Topic Queue until someone presses Generate Post. Off leaves today\u2019s post to be generated by hand too.'),
      enumField('postReadyHorizon', 'Posts written ahead', ['today', 'today-and-tomorrow'], 'today',
        'Which dates get a complete post \u2014 caption, hashtags and creative \u2014 written automatically. \u201ctoday\u201d writes only today\u2019s placed post. \u201ctoday-and-tomorrow\u201d also writes tomorrow\u2019s, but only when the posting schedule requires it: a topic is placed tomorrow and tomorrow is a posting day (\u201cAvoid weekends\u201d decides that). Every later date holds its validated topic only \u2014 no caption, image or hashtags \u2014 until Generate Post is pressed for it.'),
      num('maxAutoWrites', 'Posts written automatically per run', 3,
        'The ceiling on how many post-ready posts one run will write and illustrate. Each is a model call for the caption and another for the creative. It only ever applies to today and tomorrow, so the default covers both days across the active platforms; future dates are never written by a run.',
        { min: 0, max: 40, step: 1 }),
      bool('leadOnToday', 'Best topic is today’s post', true,
        'On, each platform’s top-ranked topic from a run takes today’s date (when today is a posting day and the platform has nothing on it), so its one post is written now.'),
      bool('freshestFirst', 'Freshest trends on the post-ready days', true,
...
const CAPTION_SKILLS: SkillSpec[] = [
  ...
  },
  {
    ...
    config: [
      ...
      bool('wholePost', 'Write the whole post in one pass', true,
        'On, the model writes the complete post in one pass \u2014 central claim first, then the hook, problem, mechanism and close in the skill\u2019s order \u2014 and the later steps keep what it wrote instead of writing their own fragments. Writing each section separately is what made bodies restate the hook and repeat themselves. Off, or when the whole-post pass fails, each section is written by its own step as before.'),
      num('linkedinMinWords', 'LinkedIn length \u2014 at least', 150,
        'The shortest a LinkedIn (and so Instagram) caption body should run, excluding hashtags. The skill\u2019s editorial default; the evidence can justify shorter, never padding.',
        { min: 40, max: 400, step: 10, unit: 'words' }),
      num('linkedinMaxWords', 'LinkedIn length \u2014 at most', 230,
        'The longest a LinkedIn (and so Instagram) caption body should run, excluding hashtags. A draft over it is asked once to tighten.',
        { min: 60, max: 600, step: 10, unit: 'words' }),
      num('facebookMinWords', 'Facebook length \u2014 at least', 100,
        'The shortest a Facebook caption body should run, excluding hashtags.',
        { min: 40, max: 400, step: 10, unit: 'words' }),
      num('facebookMaxWords', 'Facebook length \u2014 at most', 180,
        'The longest a Facebook caption body should run, excluding hashtags. A draft over it is asked once to tighten.',
        { min: 60, max: 600, step: 10, unit: 'words' }),
    ],
  },
  {
  ...
  },
  {
    ...
    config: [
      ...
      bool('etharaConnection', 'Say what Ethara does here', true,
        'On, every post carries a short line opening \u201cAt Ethara AI,\u201d just before the close: what the lab works on and how it thinks about this post\u2019s subject. It is drawn from the Knowledge Base\u2019s Brand Corpus entry for the post\u2019s domain \u2014 reinforcement learning, post-training, evaluation, agentic AI, synthetic data or infrastructure \u2014 and states only what that entry states: the lab\u2019s focus and point of view, never a product, customer, deployment, result or figure. Off, posts end on the close alone.'),
      num('etharaMaxSentences', 'Length of the Ethara line', 2,
        'How many sentences the \u201cAt Ethara AI\u201d line may run to. One is a single statement of focus; two lets it say what the lab does and why that matters for this post.',
        { min: 1, max: 3, step: 1, unit: 'sentences' }),
    ],
  },
  {
...
const IMAGE_SKILLS: SkillSpec[] = [
  ...
  },
  {
    ...
    enabledByDefault: true,
    config: [
      bool('artDirect', 'Write a bespoke composition per post', true,
        'Sends the caption to the reasoning model, which writes the composition for that specific argument before the painter renders it. Off, every post on a concept gets the same generic art direction regardless of what it says \u2014 which is what makes a feed of creatives look mechanical.'),
      text('artDirectorModel', 'Art-direction model', 'gemini-2.5-pro',
        'The reasoning model that writes the composition brief. It does not paint \u2014 no pro-tier image model is reachable on this project \u2014 it decides what the picture should be, and the image model renders it.'),
      pct('artDirectorTemperature', 'Art-direction variation', 70,
        'How much the composition may vary between posts on the same concept. Lower repeats a reliable look; higher risks drifting off the house style.'),
      num('artDirectorMaxTokens', 'Art-direction length ceiling', 400,
        'The token budget for the composition brief. A brief is two or three sentences, so this is a guard rather than a target.',
