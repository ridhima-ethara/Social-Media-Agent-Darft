...
import { Brain, Pin, Plus, Power, RefreshCw, Search } from 'lucide-react'
      ...
      {rows.length === 0 ? (
        ...
        <div className={`stagger-fade grid gap-3 ${compact ? 'md:grid-cols-2' : 'md:grid-cols-2 xl:grid-cols-3'}`}>
          {rows.map((entry, i) => (
            <article
              key={entry.id}
              className={`card anim-fade-up p-3 transition-opacity duration-[var(--dur-base)] ${entry.active ? '' : 'opacity-50'}`}
            ...
            >
              ...
              <p className="mt-1 text-[11.5px] leading-relaxed text-ink-3">{entry.content}</p>
  ...
  return (
    <>
      <PageHeader
        ...
        askPrompt="Rebuild the knowledge base"
        actions={
          <>
