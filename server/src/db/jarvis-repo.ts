 ...
 *
 * Law 7: agency requires accountability. Nothing JARVIS does is reconstructable
...
export async function latestConversation(
  workspaceId: string,
): Promise<JarvisConversationRow | null> {
  return queryOne<JarvisConversationRow>(
...
export async function listConversations(
  workspaceId: string,
  limit = 40,
): Promise<Array<JarvisConversationRow & { turn_count: number; tool_count: number }>> {
  return query<JarvisConversationRow & { turn_count: number; tool_count: number }>(
    `SELECT c.id, c.actor, c.role, c.title, c.started_at, c.last_at,
       ...
       LEFT JOIN jarvis_turns t ON t.conversation_id = c.id
       LEFT JOIN jarvis_steps s ON s.turn_id = t.id
...
export async function startConversation(
  ...
  const row = await queryOne<JarvisConversationRow>(
    `INSERT INTO jarvis_conversations (workspace_id, actor, role, title)
     VALUES ($1, $2, $3, $4)
     RETURNING id, actor, role, title, started_at, last_at`,
    [workspaceId, actor, role, title],
  )
  if (!row) throw new Error('Could not open a JARVIS conversation.')
  return row
}
...
export async function touchConversation(id: string): Promise<void> {
  await query(`UPDATE jarvis_conversations SET last_at = now() WHERE id = $1`, [id])
}
...
export interface JarvisTurnRow {
  id: string
...
export async function nextTurnSeq(conversationId: string): Promise<number> {
  const row = await queryOne<{ next: number }>(
    `SELECT COALESCE(MAX(seq), 0) + 1 AS next FROM jarvis_turns WHERE conversation_id = $1`,
    [conversationId],
  )
  return row?.next ?? 1
}
...
export async function insertTurn(t: {
  ...
  channel: 'text' | 'voice' | 'ambient' | 'cron'
  status?: TurnStatus
}): Promise<JarvisTurnRow> {
  const row = await queryOne<JarvisTurnRow>(
    `INSERT INTO jarvis_turns (conversation_id, seq, speaker, utterance, channel, status)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`,
...
export async function updateTurn(
  ...
  params.push(id)
  await query(`UPDATE jarvis_turns SET ${sets.join(', ')} WHERE id = $${i}`, params)
}

export async function listTurns(conversationId: string, limit = 60): Promise<JarvisTurnRow[]> {
  return query<JarvisTurnRow>(
...
export async function getTurn(id: string): Promise<JarvisTurnRow | null> {
  return queryOne<JarvisTurnRow>(`SELECT * FROM jarvis_turns WHERE id = $1`, [id])
}

/**
 * The pronoun resolution target: the most recent entity JARVIS or the operator
...
export async function lastReferencedEntity(
  ...
  const row = await queryOne<{ last_entity: Record<string, unknown> | null }>(
    `SELECT last_entity FROM jarvis_turns
...
export interface JarvisStepRow {
  id: string
  turn_id: string
  idx: number
  tool_id: string
...
export async function insertStep(s: {
  ...
  args: unknown
  why: string
}): Promise<JarvisStepRow> {
  const row = await queryOne<JarvisStepRow>(
    `INSERT INTO jarvis_steps (turn_id, idx, tool_id, risk, args, why, status)
...
export async function startStep(id: string): Promise<void> {
  await query(`UPDATE jarvis_steps SET status = 'running', started_at = now() WHERE id = $1`, [id])
}
...
export async function listSteps(turnId: string): Promise<JarvisStepRow[]> {
  return query<JarvisStepRow>(`SELECT * FROM jarvis_steps WHERE turn_id = $1 ORDER BY idx`, [turnId])
}

export async function listStepsForTurns(turnIds: string[]): Promise<JarvisStepRow[]> {
  if (turnIds.length === 0) return []
  return query<JarvisStepRow>(
    `SELECT * FROM jarvis_steps WHERE turn_id = ANY($1::uuid[]) ORDER BY turn_id, idx`,
    [turnIds],
  )
}
...
export interface JarvisConfirmationRow {
  id: string
  turn_id: string
  token: string
  plan: Record<string, unknown>
...
export async function mintConfirmation(c: {
  ...
  prompt: string
  ttlSeconds: number
}): Promise<JarvisConfirmationRow> {
  const row = await queryOne<JarvisConfirmationRow>(
    `INSERT INTO jarvis_confirmations (turn_id, token, plan, prompt, expires_at)
...
export async function findConfirmation(token: string): Promise<JarvisConfirmationRow | null> {
  return queryOne<JarvisConfirmationRow>(
    `SELECT * FROM jarvis_confirmations WHERE token = $1`,
    [token],
  )
}
...
export async function pendingConfirmation(
  workspaceId: string,
): Promise<JarvisConfirmationRow | null> {
  return queryOne<JarvisConfirmationRow>(
    `SELECT f.* FROM jarvis_confirmations f
       JOIN jarvis_turns t        ON t.id = f.turn_id
...
export interface JarvisBriefRow {
  id: string
  trigger: 'cron' | 'manual'
...
export async function insertBrief(b: {
  ...
  narration: string
}): Promise<JarvisBriefRow | null> {
  return queryOne<JarvisBriefRow>(
    `INSERT INTO jarvis_briefs (workspace_id, trigger, signals, recommendation, narration)
...
export async function latestBrief(workspaceId: string): Promise<JarvisBriefRow | null> {
  return queryOne<JarvisBriefRow>(
