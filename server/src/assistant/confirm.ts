 ...
 * `assistant.confirm.gate` skill cannot be disabled.
 */
...
import { NARRATION_TEMPLATES } from '../../../shared/assistant-persona'
...
export async function gatePlan(
  ...
  publish({
    type: 'assistant.confirm.required',
    turnId,
    message: prompt,
    data: {
      token,
...
export type RedeemOutcome =
  | { status: 'confirmed'; row: AssistantConfirmationRow; plan: Plan }
  | { status: 'cancelled'; row: AssistantConfirmationRow; message: string }
...
export async function redeemConfirmation(
  ...
  if (new Date(row.expires_at).getTime() <= Date.now()) {
    ...
    publish({
      type: 'assistant.confirm.resolved',
      turnId: row.turn_id,
