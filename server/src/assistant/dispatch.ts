 ...
 *
 * Every step opens a `assistant_steps` row and emits over SSE. A failed step halts
...
import { finishStep, insertStep, startStep } from '../db/assistant-repo'
...
export async function dispatchPlan(opts: DispatchOptions): Promise<DispatchResult> {
  ...
  for (let idx = 0; idx < plan.steps.length; idx += 1) {
    ...
    publish({
      ...
      turnId,
      agentId: spec.agentId ?? 'assistant',
      message: step.why,
    ...
    try {
      ...
      emitFailed(turnId, outcome, spec.agentId ?? 'assistant')
...
function emitFailed(turnId: string, outcome: StepOutcome, agentId = 'assistant'): void {
  publish({
 ...
 * papered over — The command plane never claims a success a check contradicts.
 */
