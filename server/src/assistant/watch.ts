 ...
 *
 * Ethara says something unprompted only when it has something worth saying. Six
...
import { enforceAssistantVoice } from '../../../shared/assistant-persona'
...
import { insertBrief, latestBrief, type AssistantBriefRow } from '../db/assistant-repo'
...
export interface AmbientNotice {
  ...
  message: string
  /** A one-click action: the utterance Ethara would run. */
...
export async function composeBrief(
  ...
  const address = addressOperator(opts.role, config.assistant.addressStyle)

  const narration = enforceAssistantVoice(
    [
...
export async function lastBrief(workspaceId: string): Promise<AssistantBriefRow | null> {
