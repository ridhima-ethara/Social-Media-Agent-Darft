/**
 * JARVIS'S OWN SKILLS — stage `command`
 *
 * The twelve stages of the command loop, declared and executed exactly like any
 * other agent's skills. JARVIS is not special-cased in the runtime, so every
 * knob it reads is visible in Agent Studio and every execution writes a
 * `skill_runs` row with its resolved config.
 *
 * The implementations live in `server/src/jarvis/*`; these handlers are the
 * registry's contract over them.
 */

import type { OperatorRole } from '../../../../shared/agent-contract'
import { spokenSummary } from '../../../../shared/jarvis-persona'
import { assembleSnapshot } from '../../jarvis/context'
import { parseIntent } from '../../jarvis/intent'
import { composePlan } from '../../jarvis/planner'
import { gatePlan } from '../../jarvis/confirm'
import { dispatchPlan, verifyOutcomes } from '../../jarvis/dispatch'
import { narratePlan, narrateResult, tokenise } from '../../jarvis/narrator'
...
/** What a JARVIS skill run threads between stages. */
interface JarvisPayload extends Record<string, unknown> {
  utterance?: string
  actor?: string
  role?: OperatorRole
...
/* ═══════════════════════════════════════════════════════════════════════════
   1 · jarvis.context.assemble
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.context.assemble', async (payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   2 · jarvis.intent.parse
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.intent.parse', async (payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   3 · jarvis.plan.compose
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.plan.compose', (payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   4 · jarvis.confirm.gate — CANNOT BE DISABLED
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.confirm.gate', async (payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   5 · jarvis.tool.dispatch
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.tool.dispatch', async (payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   6 · jarvis.narrate.stream
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.narrate.stream', async (payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   7 · jarvis.result.verify
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.result.verify', (payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   8 · jarvis.memory.write
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.memory.write', async (payload, ctx) => {
  ...
  // JARVIS on the same thing more than once.
  ...
  await insertKnowledgeEntry({
    ...
    source: 'JARVIS · learned from repetition',
    sources: [],
    hashtagId: null,
    confidence: 'Medium',
    origin: 'jarvis',
...
/* ═══════════════════════════════════════════════════════════════════════════
   9 · jarvis.brief.compose
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.brief.compose', async (payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   10 · jarvis.anomaly.watch
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.anomaly.watch', async (_payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   11 · jarvis.voice.transcribe
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.voice.transcribe', (payload, ctx) => {
...
/* ═══════════════════════════════════════════════════════════════════════════
   12 · jarvis.handoff.route
   ═══════════════════════════════════════════════════════════════════════════ */

registerSkill<JarvisPayload>('jarvis.handoff.route', (payload, ctx) => {
  const trigger = ctx.str('recordAsTrigger', 'jarvis')
  ...
  // through the one orchestrator, which is what makes a JARVIS-triggered run
