 ...
 *
 * JARVIS knows the state of everything without being asked, and this is where
...
import { latestBrief, lastReferencedEntity, listTurns } from '../db/jarvis-repo'
...
export function renderSnapshotText(snapshot: SituationSnapshot, maxChars: number): string {
  ...
  lines.push(`Mode: publish ${snapshot.mode.publishMode}, JARVIS ${snapshot.mode.jarvisProvider}.`)
