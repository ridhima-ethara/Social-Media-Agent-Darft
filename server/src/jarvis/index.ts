 ...
 * stage is persisted. JARVIS is not special-cased in the runtime: the same
...
/**
 * Resolves every JARVIS knob through the same layering the runtime uses:
 * registry default < workspace override < environment.
 */
async function resolveKnobs(workspaceId: string): Promise<JarvisKnobs> {
  ...
  return {
    historyTurns: Math.min(num('jarvis.context.assemble', 'historyTurns', 12), config.jarvis.historyTurns),
    includeKnowledge: bool('jarvis.context.assemble', 'includeKnowledge', true),
    maxSnapshotChars: num('jarvis.context.assemble', 'maxSnapshotChars', 6000),
    clarifyThreshold: num('jarvis.intent.parse', 'clarifyThreshold', 55),
    minConfidence: num('jarvis.intent.parse', 'minConfidence', 35),
    useModel: bool('jarvis.intent.parse', 'useModel', true),
    synonymsEnabled: bool('jarvis.intent.parse', 'synonymsEnabled', true),
    // The environment caps the registry: an operator cannot raise the plan size
    // above what the deployment allows.
    maxSteps: Math.min(num('jarvis.plan.compose', 'maxSteps', 8), config.jarvis.maxPlanSteps),
    readBeforeWrite: bool('jarvis.plan.compose', 'readBeforeWrite', true),
    explainEveryStep: bool('jarvis.plan.compose', 'explainEveryStep', true),
    ttlSeconds: num('jarvis.confirm.gate', 'ttlSeconds', config.jarvis.confirmTtlSeconds),
    alsoConfirmMutating: bool('jarvis.confirm.gate', 'alsoConfirmMutating', false),
    stepTimeoutMs: num('jarvis.tool.dispatch', 'stepTimeoutMs', 60_000),
    haltOnStepFailure: bool('jarvis.tool.dispatch', 'haltOnStepFailure', true),
    verbosity: str('jarvis.narrate.stream', 'verbosity', 'normal') as Verbosity,
    speakSummaryOnly: bool('jarvis.narrate.stream', 'speakSummaryOnly', true),
    tokenDelayMs: num('jarvis.narrate.stream', 'tokenDelayMs', 18),
    verifyEnabled: bool('jarvis.result.verify', 'enabled', true),
    verifyStrict: bool('jarvis.result.verify', 'strict', false),
    learnAfterCorrections: num('jarvis.memory.write', 'learnAfterCorrections', 2),
    askBeforeSaving: bool('jarvis.memory.write', 'askBeforeSaving', true),
    maxSpokenChars: num('jarvis.voice.transcribe', 'maxSpokenChars', 320),
    recordAsTrigger: str('jarvis.handoff.route', 'recordAsTrigger', 'jarvis'),
  }
}
...
export async function runCommand(opts: RunCommandOptions): Promise<RunCommandResult> {
  ...
  try {
    ...
    for (const token of tokenise(acknowledgement)) {
      ...
      publish({ type: 'jarvis.token', turnId: turn.id, message: token })
    }
    ...
    const narration = enforceJarvisVoice(`That failed. ${message}`)

    await updateTurn(turn.id, { status: 'failed', narration })
    opts.emit({ kind: 'error', message: narration })
    opts.emit({ kind: 'done', status: 'failed' })
    publish({ type: 'jarvis.command.finished', turnId: turn.id, message: narration, data: { status: 'failed' } })
...
async function executePlan(input: ExecuteOptions): Promise<RunCommandResult> {
  ...
  const toolCtx: Omit<ToolContext, 'prior'> = {
    ...
    actor: opts.actor,
    role: opts.role,
    // Recorded so a JARVIS-triggered run is indistinguishable from a scheduled
    // one in telemetry, apart from this label and the turn id.
    trigger: knobs.recordAsTrigger === 'jarvis' ? 'jarvis' : 'api',
    turnId,
  }
  ...
  for (const token of tokenise(narration.text)) {
    opts.emit({ kind: 'token', text: token })
    publish({ type: 'jarvis.token', turnId, message: token })
  }
  ...
  if (knobs.verifyEnabled) {
    ...
    if (verification.note) {
      finalNarration = enforceJarvisVoice(`${finalNarration} ${verification.note}`)
  ...
  publish({
    type: status === 'completed' ? 'jarvis.command.finished' : 'jarvis.command.cancelled',
    turnId,
...
async function finishConversational(
  ...
  const narration = enforceJarvisVoice(text)
