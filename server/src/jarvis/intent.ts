 ...
 *
 *   A · Model-backed, when `JARVIS_MODEL_PROVIDER=gcp` and GCP is configured.
...
import { addDays, isoDate, labelToHour, nearestPostingTime, startOfWeek } from '../agents/corpus'
...
export interface Intent {
  ...
  /** 0–100. */
  confidence: number
  /** JARVIS's own restatement, shown in the plan card. */
  restated: string
...
export async function parseIntent(opts: ParseOptions): Promise<Intent> {
  const modelReady =
    opts.useModel && config.jarvis.provider === 'gcp' && gcpText.isConfigured()
  ...
  if (!modelReady && opts.useModel) {
    grammar.parserReason =
      config.jarvis.provider === 'gcp'
        ? `JARVIS_MODEL_PROVIDER is gcp but ${gcpText.unavailableReason()}`
        : 'JARVIS_MODEL_PROVIDER is not set, so the deterministic parser is in use.'
  }
