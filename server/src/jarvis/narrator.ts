/**
 * NARRATE — renders the plan and every result as JARVIS speech.
 *
 * Two implementations, one voice. Gemini writes the prose when configured; the
 * template narrator writes it when not. Either way `enforceJarvisVoice()` is the
...
function agentNameFor(toolId: string): string {
  ...
  if (!tool?.agentId) return 'JARVIS'
  return AGENT_BY_ID[tool.agentId as AgentId]?.name ?? 'JARVIS'
}
...
export function narratePlan(plan: Plan, opts: NarratorOptions): string {
  ...
  if (plan.steps.length === 0) return enforceJarvisVoice(plan.summary)
  ...
  return enforceJarvisVoice([base, ...extras].join(' '))
}
...
export function narrateFailure(
  ...
  return enforceJarvisVoice(parts.join(' '))
}
...
export async function narrateResult(input: FinalNarrationInput): Promise<{
  ...
  const modelReady = config.jarvis.provider === 'gcp' && gcpText.isConfigured()
  if (!modelReady) {
    return {
      text: template,
  ...
  try {
    ...
    const text = enforceJarvisVoice(raw.trim())
    // A model that returns nothing usable does not get to silence JARVIS.
...
export function buildNarratorSystemPrompt(verbosity: Verbosity): string {
  ...
  return [
    'You are JARVIS, the operating intelligence of Ethara SocialAI. You are reporting to the operator.',
    '',
...
export function narrateResultTemplate(input: FinalNarrationInput): string {
  ...
  if (plan.steps.length === 0) {
    return enforceJarvisVoice(plan.summary)
  }
  ...
  if (!act) return enforceJarvisVoice(plan.summary)
  ...
  return enforceJarvisVoice(sentences.filter(Boolean).join(' '))
}
