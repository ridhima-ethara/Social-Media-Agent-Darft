 ...
 * THE TOOL SURFACE
 *
 * JARVIS acts ONLY through these handlers. There is no path from an utterance to
...
import { retrieveKnowledge } from '../../agents/skills/research'
import {
  applyInstruction,
  approveMarketing,
  buildKnowledge,
  ...
  if (missing.length > 0) {
    throw new Error(
      `Tool coverage audit failed — ${missing.length} declared tool(s) have no handler:\n` +
        ...
        `\n\nRegister them in server/src/jarvis/tools/index.ts.`,
    )
  }
...
export function capabilities(): Array<{
  ...
  return [...byAgent.entries()].map(([agentId, specs]) => ({
    agentId,
    agentName: AGENTS.find((a) => a.id === agentId)?.name ?? 'JARVIS',
    tools: specs.map((s) => ({
      id: s.id,
      name: s.name,
      summary: s.summary,
